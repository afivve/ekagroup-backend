    ...
    try:
        ...
        #  jika akses < 3 maka sql dibatasi di divisi nya saja
            ...
            if data.id_divisi != user_data.divisi:
                raise HTTPException(
                    400, detail=f"User not have access {data.id_divisi}"
                )
        elif user_data.access > 3:
            pass
        else:
            raise HTTPException(400, detail=" anda tidak mmemiliki akses ")
        ...
        result = session.execute(
            ...
            .where(Tugas.id_tugas == data.id_tugas)
        )
